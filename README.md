...
inside each lesson. They live once, in `assets/`. Every lesson points at them.
...
3. **Repository name:** type `e2i-lessons` (exactly this, all lowercase, with the hyphen).
...
2. Open the `e2i-lessons` folder on your computer.
...
5. A green box appears at the top with your website address. It will look like:
   ...
   ```
   https://YOUR-USERNAME.github.io/e2i-lessons/
   ```
...
```
https://YOUR-USERNAME.github.io/e2i-lessons/reflex-2/s01.html
                                              ↑          ↑
...
Open that Reflex 2 link on your phone. You should see the lesson, full screen.
Answer one question, close the browser completely, open the link again —
your answer should still be there.

### Step 5 — Put the button on your Odoo page

1. Open `_template/odoo-launch-button.html` in Notepad (or TextEdit).
2. Change the three things marked **CHANGE** — the link, the course name,
   the headline. Replace `YOUR-USERNAME` with your real GitHub username.
3. Select all, copy.
4. In Odoo, open the article, add an **Embed Code** block, and paste.
5. Save and leave edit mode.

Your Odoo page now has a short intro and a salmon **Bắt đầu buổi học** button.
The button opens the real lesson full screen.
...
1. On the repository page, click into the course folder — for example `reflex-2`.
2. Click **Add file** → **Upload files**.
3. Drag the new lesson file in. Name it `s05.html`, `s06.html`, and so on.
4. Type a short note in the box (`Add Reflex 2 session 5`) and click **Commit changes**.

The lesson is live within about a minute at:

```
https://YOUR-USERNAME.github.io/e2i-lessons/reflex-2/s05.html
```

Then add the launch button to the matching Odoo article.
